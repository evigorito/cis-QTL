...
###### prepare stan inputs
                    ...
                    stan.in1<-mclapply(rec.full$id, function(i) stan.trecase.eff(rp.1r=rp.r[i,,drop=FALSE], rp.f, f.ase, rs.hap=rs.full[id==i,], geno.exp=filt.counts[names(filt.counts)==i][[1]],min.ase, min.ase.het))
                    ...
                    if(length(stan.in1) >= 1){
                        ...
                        full.sum <- stan.bt(x=stan.full,y= "bj",rtag=r.tag,model="trec-ase", nhets=rec.full[id%in% names(stan.full),nhet],ASE.het=ASE.hets,gene=gene)
    ...
    if(nrow(rsnps.ex)>0) {
        ...
        if(!!length(id.keep)){            
            ...
            neg.sum <- stan.bt(x=stan.neg, y="bj",rtag=r.tag,model="trec", nhets=rec.rs[id %in% id.keep,nhet],gene=gene)
            if( exists("full.sum")){
...
#' Run Btrecase with known rsnp GT, version 2, preparing inputs more efficiently
#'
#' This function allows you to run Btrecase for one gene and multiple pre-selected snps. When there is no enough information to ASE counts or rSNP is not in the reference panel, the function will run bayesian negative binomial model only.
#' @param gene gene id for the gene to run
#' @param chr chromosome where the gene is, example chr=22
...
###### prepare stan inputs
    ...
    if(nrow(rsnps.ex)>0) {
        ...
        if(length(id.keep)){            
            ...
            stan.neg <-  mclapply(in.neg, function (i) {
                if(is.null(prob)){
                    s <- summary(sampling(mod2,data=i, cores=1, refresh=0), pars='bj', use_cache=F)$summary
                } else {
                    s <- summary(sampling(mod2,data=i, cores=1, refresh=0), pars='bj', use_cache=F, probs=probs)$summary
                }
...
#' Run Btrecase with known rsnp GT, version 2, with input for trecase already prepared
#'
#' This function allows you to run Btrecase for one gene and multiple pre-selected snps. When there is no enough information to ASE counts or rSNP is not in the reference panel, the function will run bayesian negative binomial model only.
#' @param gene gene id for the gene to run
#' @param snps either all in input file or character vector with pos:ref:alt allele for each snp to select snps from input, defaults to all
#' @param covariates path to matrix of covariates prepared in inputs.R, if no covariates, covariates =1, default
#' @param out path to save outputs, default to current directory
#' @param prefix optional prefix for saving tables, if NULL gene_id.eqtl will be used
#' @param stan.in full path to file with input for trecase model (stan.in1)
#' @param tag whether input was run with tags, defaults to yes
#' @param prob  number p∈(0,1) indicating the desired probability mass to include in the intervals, defaults to 0.95
#' @keywords bayesian trecase known genotype regulatory snp
#' @export
#' @return data.table with summary of gene-snp associations. Saves the summary table in "out" dir as /out/prefix.main.txt. When using tags, saves /out/prefix.tags.lookup.txt. Saves a table of excluded rsnps from trecase model.
...
btrecase.in.gt2 <- function(gene, snps='all',covariates=1, out=".", prefix=NULL, stan.in, tags="yes", prob=NULL) {
    ...
    r.tag <- switch(tags=="yes" & any(snps=="all"),"yes") ## to output results after stan, when tag.threshold is char, returns NULL

    ## get stan inputs
    stan.in1 <- readRDS(stan.in)
    if(snps != 'all'){
        stan.in1 <- stan.in1[snps]
    }
    
    print(any(sapply(stan.in1, is.character)))
    ...
    ## count number of hets with sufficient ase counts to input in final report
    ASE.hets <- sapply(stan.in1, function(i) nrow(i$gm[abs(g.ase)==1,]))
    full.sum <- stan.bt(x=stan.full,y= NULL,rtag=r.tag,model="trec", ASE.het=ASE.hets,gene=gene,probs=probs)
    if(!is.null(prefix)) {             
        write.table(full.sum, paste0(out,"/",prefix,".stan.summary.txt"), row.names=FALSE)
    } else {
        write.table(full.sum,paste0(out,"/",gene,".stan.summary.txt"), row.names=FALSE)
