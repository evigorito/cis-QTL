...
#' Run Btrecase with unknown rsnp GT and NO missing values for GT fsnps
#'
...
#' @param min.ase.snp minum number of ASE counts for a single snp to be considered, for a particualr individual
...
btrecase.nogt.v1 <- function(gene, chr, snps=5*10^5,counts.f,covariates=1,e.snps,gene.coord,vcf,le.file,h.file,population=c("EUR","AFR", "AMR", "EAS",  "SAS", "ALL"), maf=0.05, min.ase=5,min.ase.snp=5,min.ase.n=5,tag.threshold=.9,q.test="no", out=".", prefix=NULL, model="/home/ev250/Bayesian_inf/trecase/Scripts/stan_eff/neg.beta.noGT.rsnp.priors.eff2.stan") {
  ...
  num <- list(maf,min.ase,min.ase.snp, min.ase.n)
  names(num) <-  c("maf", "min.ase","min.ase.snp", "min.ase.n")
  ...
  if(covariates !=1){
    ...
    rownames(covariates)=names(counts.g)[2:ncol(counts.g)]
  }
  ...
  if(is.character(c.ase)) stop(c.ase)
    ...
    snps.ex=rbind(snps.ex, data.table(id=fsnps$id[w], reason=rep("exonic snp is not in reference panel", length(w))))
...
##############################  Stan inputs ###############################
    ...
    stan.noGT<-mclapply(1:nrow(rp.r), function(i) stan.trecase.noGT.eff(counts.g, rp.1r=rp.r[i,,drop=FALSE], rp.f, f.ase, c.ase , min.ase, min.ase.n))
    ...
    if(length(stan.noGT) >= 1){
        ...
        full.sum <- stan.bt(x=stan.full,y= "bj",rtag=r.tag,model="trec-ase",gene=gene)
        if(!is.null(prefix)) {             
...
#' Run Btrecase with unknown rsnp GT: aux function to prepare stan inputs
#'
#' This function allows you to prepare stan-inputs for running Btrecase for one gene and multiple pre-selected snps. 
#' @param gene gene id for the gene to run
#' @param chr chromosome where the gene is, example chr=22
#' @param snps either cis-window or character vector with pos:ref:alt allele for each snp, defaults to cis-window
#' @param counts.f path to file with filtered counts: rows genes, first col gene_id followed by samples, prepared in inputs.R
...
btrecase.nogt.in.v1 <- function(gene, chr, snps=5*10^5,counts.f,covariates=1,e.snps,gene.coord,vcf,le.file,h.file,population=c("EUR","AFR", "AMR", "EAS",  "SAS", "ALL"), maf=0.05, min.ase=5,min.ase.snp=5,min.ase.n=5,tag.threshold=.9,q.test="no", out=".", prefix=NULL, model="trecase") {
  ...
  if(is.numeric(snps)) {
    ...
    if(!nrow(rp)) stop("No snps extracted from the reference panel")
    w=which(!snps %in% rownames(rp))
    if(length(w)==length(snps)) stop(print("None of the snps ", snps, "are in the reference panel"))   
    if(length(w)) {
        rp.r=rp[snps[-w],,drop=FALSE] ## only rsnps requested and in ref panel
        snps.ex=rbind(snps.ex, data.table(id=snps[w], reason=rep("snp not in reference panel",length(w))))
    } else {
    ...
    ## only use fsnps in ref panel
    f.ase <- gt.as[id %in% rownames(rp),]
    
    rp.f=rp[rownames(rp) %in% fsnps$id, ,drop=FALSE] ## exonic snps to work with
    c.ase=c.ase[,unlist(lapply(f.ase$id, function(i) grep(i, colnames(c.ase), value = TRUE)))]
    ## Check if there are sufficient hets with enough ASE counts if removing fsnps not in reference panel
    if(nrow(rp.f) < nrow(fsnps)) {
        c.ase = filt.fsnp(c.ase,ase=min.ase,min.ase.snp,n=min.ase.n, rem=NULL)
...
#' Run Btrecase with unknown rsnp GT and NO missing values for GT fsnps, version 2, improved stan input preparation
#'
#' This function allows you to run Btrecase for one gene and multiple pre-selected snps. 
#' @param gene gene id for the gene to run
#' @param chr chromosome where the gene is, example chr=22
...
btrecase.nogt.v2 <- function(gene, chr, snps=5*10^5,counts.f,covariates=1,e.snps,gene.coord,vcf,le.file,h.file,population=c("EUR","AFR", "AMR", "EAS",  "SAS", "ALL"), maf=0.05, min.ase=5,min.ase.snp=5,min.ase.n=5,tag.threshold=.9,q.test="no", info=0.3, out=".", prefix=NULL, model="/home/ev250/Bayesian_inf/trecase/Scripts/stan_eff/neg.beta.noGT.rsnp.priors.eff2.stan") {
  ...
  if(is.character(c.ase)) stop(c.ase)
    ...
    ## Extract haps for fsnps and rsnp from reference panel ########
    if(is.numeric(snps)) {
        cis_window <- tryCatch({cl_coord(file=gene.coord,chr=chr,gene=gene,cw=snps)}, error=function(e) {paste("Gene " ,gene, "and chromosome", chr, "are incompatibles in gene.coord input")})
        if(is.character(cis_window)) stop(cis_window)
        rp <- haps.range(file1=le.file,file2=h.file,cw=cis_window,population, maf)
        if(!nrow(rp)) stop("No snps extracted from the reference panel")
        rp.r=rp
...
##############################  Stan inputs ###############################
    ...
    ## fsnps
    stan.f <- stan.fsnp.noGT.eff(rp.f,f.ase,c.ase,NB="yes", min.ase,min.ase.n)

    if(is.character(stan.f)) stop(stan.f)

    ## add rsnp
    
    stan.noGT<-mclapply(1:nrow(rp.r), function(i) stan.rsnp.noGT.eff(counts.g, rp.f, rp.1r=rp.r[i,,drop=FALSE], stan.f))
    names(stan.noGT) <- rownames(rp.r)
    ...
    if(length(stan.noGT) >= 1){
        ...
        eaf.t <- snp.eaf(le.file,names(stan.noGT2),population)
        full.sum <- stan.bt(x=stan.full,y= "bj",rtag=r.tag,model="trec-ase",gene=gene,EAF=eaf.t, info=info.ok, nfsnps=nchar(unlist(strsplit(colnames(stan.f$n[[1]]), ","))[1]) )
        if(!is.null(prefix)) {             
...
#' Run Btrecase with unknown rsnp GT and missing values for GT fsnps from pre-built stan input
#'
#' This function allows you to run Btrecase for one gene and multiple pre-selected snps, pre-built inputs. 
#' @param gene gene id for the gene to run
#' @param chr chromosome where the gene is, example chr=22
#' @param covariates path to matrix of covariates prepared in inputs.R, if no covariates, covariates =1, default
#' @param tag.threshold numeric with r2 threshold (0-1) for grouping snps to reduce the number of running tests, to disable use "no"
...
#' @export
#' @return data.table with summary of gene-snp associations. Saves the summary table in "out" dir as /out/prefix.main.txt. When using tags, saves /out/prefix.tags.lookup.txt. Saves a table of excluded rsnps from trecase model.
#' btrecase.nogt.rna.in()

btrecase.nogt.rna.in <- function(gene, chr, covariates=1,counts.f,tag.threshold=.9, out=".", prefix=NULL, model="/home/ev250/Bayesian_inf/trecase/Scripts/stan_eff/neg.beta.noGT.rsnp.priors02.eff2.stan"){
    if(is.null(prefix)){
        stan.noGT <- readRDS(paste0(out,"/", gene, ".noGT.stan.in.rds"))
    } else {
        stan.noGT <- readRDS(paste0(out,"/", prefix, ".noGT.stan.in.rds"))
    }
    ...
    if(exists("stan.trec")){ ## trec model
        ...
        names(neg.only) <- names(stan.trec)
        neg.sum <- stan.bt(x=neg.only,y= "bj",rtag=r.tag,model="trec",gene=gene)
