...
if(nrow(counts.g) == 0){  ## if no counts, stop running
    ...
    stop(paste("NO COUNTS FOR", gene))
        ...
        ## extract GT and ASE, remove non-informative fsnps
...
##################### Extract haps for fsnps and rsnp from reference panel ########
                ...
                ref <- haps.range(file1=le.file,file2=h.file, cw=cis_window,population="EUR",maf=0.01)
...
#### fsnps from reference panel
                ...
                ## I have haplotypes from reference panel but need genotypes for GUESSFM, take a sample for haplotpe 1 and haplotype2 and get genotypes, thye run GUESSFM

                g <- Reduce('+', lapply(1:2, function(i) ref[,sample(1:nrow(ref), 500, replace=TRUE)]))
                ...
                tags object, containing 4761 SNPs in 2266 groups.
